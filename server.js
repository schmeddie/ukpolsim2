...
app.get('/api/parliament/summary', (req, res) => {
  const db = getDb();
...
app.post('/api/emails/read-all', (req, res) => {
  const db = getDb();
...
app.get('/api/news', (req, res) => {
  const db = getDb();
  const news = db.prepare('SELECT * FROM news_items ORDER BY id DESC LIMIT 20').all();
  res.json(news);
});

// ─── Backstory options ────────────────────────────────────────────────────────

app.get('/api/backstories', (req, res) => {
  res.json([
